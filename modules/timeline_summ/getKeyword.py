...
def make_morph_tag_sentence(sentence):
    morp_element_list = []
    temp = []
    # get morp
    morp_elements = [morp_info['lemma'] for morp_info in sentence['morph']]
    pos_elements = [morp_info['type'] for morp_info in sentence['morph']]

    for i in range(len(morp_elements)):
        temp.append(str(morp_elements[i]) + '/' + str(pos_elements[i]) + '_')
...
def main(timelineSet, doc_save_list, query):
    ...
    for tmpdic in doc_save_list:
        ...
        for sentence in sentences:
            morph_tag = make_morph_tag_sentence(sentence)
