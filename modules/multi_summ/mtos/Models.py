...
class RNNDecoderBase(nn.Module):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        # weight for drop swap or replace swap
        
        #self.type_weight = nn.Linear(hidden_size*2, 1, bias=True)

        # Set up the context gate.
        self.context_gate = None
        if context_gate is not None:
            self.context_gate = mtos.modules.context_gate_factory(
        ...
        self.attn = mtos.modules.GlobalAttention(
        ...
        if copy_attn and not reuse_copy_attn:
            self.copy_attn = mtos.modules.GlobalAttention(
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _build_rnn(self, rnn_type, input_size,
        ...
        if rnn_type == "LSTM":
            stacked_cell = mtos.modules.StackedLSTM
        else:
            stacked_cell = mtos.modules.StackedGRU
...
class NMTModel(nn.Module):
    ...
    def forward(self, src, tgt, lengths, dec_state=None, sort=False):
        ...
        if sort:
            #print("model line:610 sort")
            sorted_src_lengths, sorted_indices = torch.sort(lengths, descending=True)
            sorted_indices = torch.autograd.Variable(sorted_indices)
            sorted_src = torch.index_select(src, 1, sorted_indices)
            ...
            enc_final, memory_bank = self.encoder(sorted_src, sorted_src_lengths)        
            memory_bank = torch.index_select(memory_bank, 1, reversed_indices) 
            if isinstance(enc_final, tuple):
                enc_final = (torch.index_select(enc_final[0], 1, reversed_indices), torch.index_select(enc_final[1], 1, reversed_indices))
