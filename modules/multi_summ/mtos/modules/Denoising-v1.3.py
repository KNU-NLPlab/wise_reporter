...
class Denoising():
  ...
  def __init__(self, noising_prob=0.1,  null_word_index=2, padding_index=1):
    ...
    #self.noise_list = list(range(6))
    #self.noise_list = list(range(5))
    self.noise_list = [3,4]
    # 0: drop
    # 1: swap
    # 2: rep
  ...
  def seperate_noising_all(self, src, src_lengths):
    ...
    for i in range(batch_size):
        ...
        noised_src, altered_length, noised_type = self.random_noise_from_all(src[:,i,:].data, src_lengths[i:i+1])
  ...
  def random_noise_from_all(self, src, src_lengths):
    """
    # src : len * batch * 1
    self.flag = 1 - self.flag
    if self.flag == 1: 
      ...
      return noised_src, noised_src_lengths, 1
    """
    selected_noise = random.sample(self.noise_list,1)[0]
    
    if selected_noise == 0: 
      noised_src, noised_src_lengths, noise_cnt  = self.noise_drop(src, src_lengths)
    elif selected_noise == 1: 
      noised_src, noised_src_lengths, noise_cnt  = self.noise_swap(src, src_lengths)
    elif selected_noise == 2: 
      noised_src, noised_src_lengths, noise_cnt  = self.noise_replace(src, src_lengths)
    elif selected_noise == 3: 
      noised_src, noised_src_lengths, noise_cnt  = self.noise_drop_swap(src, src_lengths)
    elif selected_noise == 4: 
      noised_src, noised_src_lengths, noise_cnt = self.noise_replace_swap(src, src_lengths)  
    elif selected_noise == 5: 
      noised_src, noised_src_lengths, noise_cnt = self.noise_drop_replace(src, src_lengths)  

    return noised_src, noised_src_lengths, selected_noise
  ...
  def noise_drop(self, src, src_lengths):
    """
    Inject noise into src data and return noised data
    noise scheme is "drop"
  ...
  def noise_swap(self, src, src_lengths):
    """
    Inject noise into src data and return noised data
    noise scheme is "swap"
  ...
  def noise_replace(self, src, src_lengths):
    """
    Inject noise into src data and return noised data
    noise scheme is "replace"
...
#      noised_src_lengths = src_lengths
  ...
  def noise_drop_swap(self, src, src_lengths):
    ...
    """
    noised_src, noised_src_lengths, drop_noised_cnt = self.noise_drop(src, src_lengths)
    noised_src, noised_src_lengths, swap_noised_cnt = self.noise_swap(noised_src, noised_src_lengths)

    return noised_src, noised_src_lengths, (drop_noised_cnt, swap_noised_cnt)
  ...
  def noise_replace_swap(self, src, src_lengths):
    ...
    """
    noised_src, noised_src_lengths, rep_noised_cnt = self.noise_replace(src, src_lengths)
    noised_src, noised_src_lengths, swap_noised_cnt = self.noise_swap(noised_src, noised_src_lengths)

    return noised_src, noised_src_lengths, (rep_noised_cnt, swap_noised_cnt)

  def noise_drop_replace(self, src, src_lengths):
    """
    Inject noise into src data and return noised data
    noise scheme is "drop+replace"
    ...
    """
    noised_src, noised_src_lengths, drop_noised_cnt = self.noise_drop(src, src_lengths)
    noised_src, noised_src_lengths, replace_noised_cnt = self.noise_replace(noised_src, noised_src_lengths)

    return noised_src, noised_src_lengths, (drop_noised_cnt, replace_noised_cnt)
