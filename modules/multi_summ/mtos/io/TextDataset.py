...
from mtos.io.DatasetBase import (ONMTDatasetBase, UNK_WORD,
...
class TextDataset(ONMTDatasetBase):
    ...
    @staticmethod
    def make_text_examples_nfeats_tpl_for_ol_test(input_msg, truncate, side):
        """
        Args:
        ...
        # code for online test that process instant input msg not file
        def read_text_from_input_msg(input_msg, truncate, side):
            """
            Args:
                input msg (str): input msg for online test.
                truncate (int): maximum sequence length (0 for unlimited).
            ...
            """
            for i, line in enumerate([input_msg, input_msg]):
                line = input_msg.strip().split()
                if truncate:
                    line = line[:truncate]
        ...
        examples_nfeats_iter = \
            read_text_from_input_msg(input_msg, truncate, side)
