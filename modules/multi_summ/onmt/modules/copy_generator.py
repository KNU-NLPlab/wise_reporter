...
class CopyGeneratorLoss(nn.Module):
    ...
    def forward(self, scores, align, target):
        ...
        vocab_probs = scores.gather(1, target.unsqueeze(1)).squeeze(1)  # 전체 vocab에서의 아웃이 진짜 타겟일 나올 확률

        # probability of tokens copied from source
        copy_ix = align.unsqueeze(1) + self.vocab_size  # 원문 vocab를 전체 vocab 뒤에 오도록 인덱스 조정
        copy_tok_probs = scores.gather(1, copy_ix).squeeze(1)  # score에서 원문에서 단어를 뽑을 확률만 뽑아냄
        # Set scores for unk to 0 and add eps
        copy_tok_probs[align == self.unk_index] = 0  # unk_index이면 확률을 0으로 만듬.
        copy_tok_probs += self.eps  # to avoid -inf logs  # log 씌웠을 때 에러 안나도록 아주 작은값 더해줌
        ...
        probs = torch.where(
            non_copy, copy_tok_probs + vocab_probs, copy_tok_probs  # copy면 원래 답이 나올 확률이 copy_tok_probs 에서만
                                                                    # 나오고, noncopy면 원래 답이 나올 확률은
                                                                    # 전체 vocab에서 아웃이 진짜 타겟일 확률 vocab_probs
                                                                    # + 원문 vocab에서 아웃이 진짜 타겟이 확률
                                                                    # copy_tok_probs이다.
        )
        ...
        # eos penalty
        loss = loss * (
                    scores.gather(1, torch.tensor([30800 for _ in range(target.size(0))]).cuda().unsqueeze(1)).squeeze(
                        1) * non_copy.type(torch.float32) * ((target == 30800) == 0).type(torch.float32) + 1) * 2
