...
class Trainer(object):
    ...
    def train(self,
        ...
        # valid_iter = None
        # import random
        # diter = pickle.load(open('./data/duc_multi/duc2002_cnndmduc_vocab_400cut.pkl', 'rb')) + \
        #         pickle.load(open('./data/duc_multi/duc2004_cnndmduc_vocab_400cut.pkl', 'rb'))
        # while True:
        #     random.shuffle(diter)
        #     for i, batchess in enumerate(diter):
        for i, (batches, normalization) in enumerate(
            self._accum_batches(train_iter)):
            # batches = [bbbb(batchess)]
            # normalization = 1
...
class bbbb:
    def __init__(self, batch):
        self.batch_size = 1
        # self.raw_src = batch['src_map']
        # self.raw_tgt = batch['src_map']
        ...
        # self.itos = batch['src_map']
        # self.stoi = batch['src_map']
        self.alignment = batch['alignment'].to(torch.device('cuda'))
        self.tgt = batch['tgt'].to(torch.device('cuda'))

    def makeSrcmap(self, data, lengths):
        data = torch.cat([data.view(lengths.size(0), -1)[i][:lengths[i]] for i in range(lengths.size(0))]).view(1, -1)
