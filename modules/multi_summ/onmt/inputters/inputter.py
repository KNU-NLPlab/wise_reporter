...
def make_src(data, vocab):
    ...
    for i, a in enumerate(data):
        if len(a) < 1:
            data[i] = torch.tensor([0, 1], dtype=torch.int64)
...
def get_fields(
    ...
    if bert:
        src_field_kwargs = {"n_feats": n_src_feats,
                            ...
                            "pad": '[PAD]', "bos": None, "eos": None, "bert": True,
    ...
    if bert:
        tgt_field_kwargs = {"n_feats": n_tgt_feats,
                            ...
                            "pad": pad, "bos": bos, "eos": eos, "bert": True,
    ...
    if bert:
        ...
        fields["src"][0][1].base_field.tokenize = BertTokenizer.from_pretrained('bert-large-uncased')
        fields["tgt"][0][1].base_field.tokenize = BertTokenizer.from_pretrained('bert-large-uncased')
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[30522] = '<s>'
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[30523] = '<t>'
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[30524] = '</t>'
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[30525] = '</s>'
        # fields["src"][0][1].base_field.tokenize.vocab['<s>'] = 30522
        # fields["src"][0][1].base_field.tokenize.vocab['<t>'] = 30523
        # fields["src"][0][1].base_field.tokenize.vocab['</t>'] = 30524
        # fields["src"][0][1].base_field.tokenize.vocab['</s>'] = 30525
    if korean_subword:
        from bert_eojeol_pytorch.src_tokenizer.tokenization import BertTokenizer
        fields["src"][0][1].base_field.tokenize = BertTokenizer('./bert_eojeol_pytorch/vocab.korean.rawtext.list')
        fields["tgt"][0][1].base_field.tokenize = BertTokenizer('./bert_eojeol_pytorch/vocab.korean.rawtext.list')
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[-1] = '<s>'
        fields["tgt"][0][1].base_field.tokenize.ids_to_tokens[-1] = '</s>'
...
def build_dataset(fields, data_type, src, src_reader,
                  ...
                  src_seq_len=50, tgt_seq_len=50, use_filter_pred=True, bert=False, morph=False, korean_subword=False):
    ...
    return dataset_classes[data_type](
        ...
        filter_pred=filter_pred, bert=bert, morph=morph, korean_subword=korean_subword)
