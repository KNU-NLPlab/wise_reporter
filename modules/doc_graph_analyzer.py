...
class DocGraphAnalyzer():
    ...
    def extract_subtopic_info(self, doc_info_list, top_doc_num=10, top_keyword_num=10, hierarchy=True, cutting_level=2):
        ...
        raw_doc_obj = self._generate_doc_raw_obj(node_cnt_list, node2freq, node2df, node2idx, edge_cnt_list, edge2freq,
                                                 edge2idx)
        ...
        _ = docgraph_obj.GenerateGraph(opt, raw_doc_obj.idx2keyword, raw_doc_obj.idx2edge,
                                       raw_doc_obj.edgeidx2frequency)
        dict_subgraph = docgraph_obj.FindCommunity(opt, cutting_level)
        ...
        self.dict_subgraph = dict_subgraph
    ...
    ## modified by sspark ##
    def save_result_tojson(self, json_path='output.json'):
        '''
        save result to json file        
        Args:
            json_path (str): path to generate output
        '''
        dict_subgraph = self.dict_subgraph
    ...
    def make_viz_data(self, top_doc_info_list, node_cut=50, edge_cut=20, file_name='detail.json'):
        ...
        node_idx_list = [sorted_keyword_list[:node_cut] for sorted_keyword_list in
                         self.analsis_obj.sorted_keyword_insubtopics]
        # 살아남을 Edge의 node들 (Edge의 두 Node 모두 여기 있어야 Edge 살림)
        edge_node_idx_list = sum(
            [sorted_keyword_list[:edge_cut] for sorted_keyword_list in self.analsis_obj.sorted_keyword_insubtopics], [])
    ...
    def _make_node_counter(self, doc_info_list):
        ...
        joined_noun_cnt, joined_noun_df_cnt = self._cut_vocab(joined_noun_cnt, joined_noun_df_cnt, doc_size,
                                                              *self.df_bound)
        ...
        node_cnt_list = [Counter({word2idx[word]: cnt for word, cnt in node_cnt.items() if word in word2idx}) for
                         node_cnt in node_cnt_list]
