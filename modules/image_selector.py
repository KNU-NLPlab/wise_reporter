        ...
        if query not in check_query_exist:  # case 1 : if query is new - download images & caption from google, scouter caching(url, caption), local caching(image path, caption)
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    ...
                    raise
            localcache_img_name = 'localcache/%s_img.txt'%(query)
            localcache_caption_name = 'localcache/%s_caption.txt'%(query)
            ...
            try:
                ...
                localcache_img_name = 'localcache/%s_img.txt'%(query)
                localcache_caption_name = 'localcache/%s_caption.txt'%(query)
                file_list, image_list, caption_list = image_caption_get(localcache_img_name, localcache_caption_name)
            except IOError:
